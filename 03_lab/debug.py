...
def read_log():
    try:
        lines = sys.stdin.readlines() # read every line individually
        ...
        entries = [
            (
                ...
                str(entry[4]).strip(),
                str(entry[5]).strip(),
                int(entry[6]) if entry[6].isdigit() else entry[6],
                int(entry[7]) if entry[7].isdigit() else entry[7],
            )
...
def get_successful_reads(entries):
    successful_reads = [
        entry
        for entry in entries
        if isinstance(entry[7], int) and 200 <= entry[7] < 300
    ]  # first check = .int, 2nd condition of 8th entry
...
def get_failed_reads(entries, separate_lists=False):
    http_4xx = [
        entry
        for entry in entries
        if isinstance(entry[7], int) and 400 <= entry[7] < 500
    ]
    http_5xx = [
        entry
        for entry in entries
        if isinstance(entry[7], int) and 500 <= entry[7] < 600
    ]
...
def get_entries_by_code(entries, status_code):
    ...
    entries_with_code = [
        entry
        for entry in entries
        if isinstance(entry[7], int) and entry[7] == status_code
    ]
    logging.info(
        f"Number of entries with status code {status_code}: {len(entries_with_code)}"
    )
...
def get_entries_by_extension(entries, extension):
    entries_with_extension = [
        entry
        for entry in entries
        if isinstance(entry[6], str) and entry[6].endswith(extension)
    ]
    logging.info(
        f'Number of entries with extension "{extension}": {len(entries_with_extension)}'
    )
...
def run():
    ...
    entries_with_extension = get_entries_by_extension(entries, "gif")
